   ...
   class http_plugin_impl: std::enable_shared_from_this<http_plugin_impl> {
      public:
         ...
         struct abstract_conn_impl : public detail::abstract_conn {
            abstract_conn_impl(detail::connection_ptr<T> conn, std::shared_ptr<class http_plugin_impl> impl)
            :_conn(std::move(conn)),
            _impl(impl)
            {
          ...
          */
         template<typename T>
         static detail::abstract_conn_ptr make_abstract_conn_ptr( detail::connection_ptr<T> conn, std::shared_ptr<class http_plugin_impl> impl ) {
         ...
         void handle_http_request(detail::connection_ptr<T> con) {
            try {
               ...
               auto abstract_conn_ptr = make_abstract_conn_ptr<T>(con, this->shared_from_this());
